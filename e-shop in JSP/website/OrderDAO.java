...
public class OrderDAO{
	...
	//Returns all orderitems base on order_id
	public List<ArrayList<Integer>> getOrderItems(int order_id) throws Exception {
		DB temp = new DB();
		Connection con = temp.getConnection();
		List<ArrayList<Integer> > items =
                  new ArrayList<ArrayList<Integer> >();

		ResultSet rs;

		try{
				String findItems= "SELECT product_id,quantity FROM orderitems WHERE OrderCode=(?);";
				PreparedStatement stmt = null;
				stmt = con.prepareStatement(findItems);
				stmt.setInt(1, order_id);
				rs = stmt.executeQuery();
				while(rs.next()) {
					ArrayList<Integer> tempitems = new ArrayList<Integer>();
					tempitems.add(rs.getInt("product_id"));
					tempitems.add(rs.getInt("quantity"));
			    	items.add(tempitems);
			    }
				con.close();
				return items;
			...
			con.close();
			throw new Exception("An error occured while getting the Order: " + e.getMessage());
		}
